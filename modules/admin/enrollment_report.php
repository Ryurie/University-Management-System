<?php
// modules/admin/enrollment_report.php
session_start();
...
try {
    // Kunin ang total count
    $total_enrolled = $pdo->query("SELECT COUNT(*) FROM enrollments")->fetchColumn() ?: 0;
    
    // Kunin ang buong listahan
    $stmt = $pdo->query("
        SELECT e.enrollment_date, s.student_number, s.first_name, s.last_name, 
               c.section_name, crs.course_code, crs.course_name
        FROM enrollments e
...
<style>
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
    ...
    :root {
        ...
        --danger-color: #ef4444; --border-color: rgba(0,0,0,0.06);
...
<div class="main-vanilla-wrapper">
    <nav class="custom-nav">
        ...
        <div class="nav-menu">
            <a href="enrollment_report.php" class="nav-btn" style="color: var(--primary-color);">📄 Enrollment Report</a>
            <a href="finance.php" class="nav-btn">💰 Finance</a>
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title">Master Enrollment Report</h1>
            <p style="color: gray; margin-top:5px;">Total Enrolled Students: <strong style="color: var(--primary-color);"><?= number_format($total_enrolled) ?></strong></p>
        </div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    <?php if(empty($enrollments)): ?>
                        <tr><td colspan="5" style="text-align:center; padding: 40px; color: gray;">No enrollment records found.</td></tr>
                    <?php else: ?>
                        <?php foreach($enrollments as $row): ?>
                            <tr>
                                <td style="color: gray; font-size: 14px;"><?= date('M d, Y h:i A', strtotime($row['enrollment_date'])) ?></td>
                                <td><strong><?= htmlspecialchars($row['student_number']) ?></strong></td>
                                <td style="font-weight: 600; color: var(--primary-color);"><?= htmlspecialchars($row['last_name']) ?>, <?= htmlspecialchars($row['first_name']) ?></td>
                                <td><?= htmlspecialchars($row['section_name']) ?></td>
                                <td><span style="background: var(--bg-color); padding: 4px 8px; border-radius: 6px; font-size: 12px; font-weight:bold; border: 1px solid var(--border-color);"><?= htmlspecialchars($row['course_code']) ?></span> - <small style="color:gray;"><?= htmlspecialchars($row['course_name']) ?></small></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
